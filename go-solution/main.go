...
const (
	deafultMeasurementsPath = "../measurements.txt"
	maxNameLen              = 100
	maxNameNum              = 10000

	// TODO tune for M1 po
	defaultParseChunkSizeMB = 32
	mb                      = 1024 * 1024
)
...
// parseFloatFast is a high performance float parser
// using the assumption that the byte silce will
// alway have a single decimal digit
func parseFloatFast(bs []byte) float64 {
	...
	v := float64(bs[len(bs)-1]-'0') / 10
	place := 1.0
	for i := len(bs) - 3; i >= intStartIdx; i-- {
...
func main() {
	...
	measurementsPath := deafultMeasurementsPath
	if len(os.Args) > 1 {
